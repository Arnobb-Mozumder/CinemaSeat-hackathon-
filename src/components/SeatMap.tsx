...
export const SeatMap: React.FC<SeatMapProps> = ({
  ...
  return (
    <div className="w-full space-y-8 glass-container rounded-2xl p-4 sm:p-8 shadow-2xl">
      {/* Curved Cinema Screen Header */}
      <div className="max-w-3xl mx-auto space-y-3 text-center">
        <div className="relative pt-2">
          {/* Curved glowing screen line */}
          <div className="h-2.5 w-full bg-gradient-to-r from-emerald-600/20 via-emerald-500 to-emerald-600/20 rounded-t-[100%] border-t-2 border-emerald-500 shadow-[0_-8px_24px_rgba(16,185,129,0.4)]" />
          <div className="absolute inset-0 bg-gradient-to-b from-emerald-500/10 to-transparent pointer-events-none blur-sm h-12" />
        </div>
        <span className="text-xs uppercase tracking-[0.2em] font-extrabold text-emerald-400 block pt-1 font-mono">
          CINEMA SCREEN
        </span>
      </div>
      ...
      <div className="overflow-x-auto pb-6 pt-2 scrollbar-thin scrollbar-thumb-neutral-800">
        <div className="min-w-[560px] max-w-4xl mx-auto space-y-3 sm:space-y-4 px-2">
          {rows.map((rowLetter) => {
