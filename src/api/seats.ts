...
function mapBackendSeatMapToSpa(
  ...
  const show: ShowDetails = {
    ...
    movieTitle: data.show.movieTitle || 'Selected Movie',
    moviePosterUrl: data.show.moviePosterUrl || '/images/brand_new_day.webp',
    durationMinutes: data.show.durationMinutes || 135,
  ...
  // Keep MOCK_SHOWTIMES in sync if fallback occurs
  const availableCount = seats.filter((s) => s.status === 'available').length;
  const mockShowtime = MOCK_SHOWTIMES.find((s) => s.id === showId);
  if (mockShowtime) {
    mockShowtime.availableSeatsCount = availableCount;
  }
