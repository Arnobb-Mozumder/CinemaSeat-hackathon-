...
/**
 * GET /holds/{holdId}
 * Fetches hold details by holdId
 */
export async function getHold(holdId: string): Promise<HoldResponse> {
  if (!apiClient.isMockMode) {
    try {
      return await apiClient.request<HoldResponse>(`/holds/${holdId}`);
    } catch (err) {
      console.warn('Backend API unavailable for fetching hold:', err);
    }
  }
...
/**
 * DELETE /holds/{holdId}
 * Releases/cancels an active seat hold on the server.
 */
export async function releaseHold(holdId: string): Promise<void> {
  if (!apiClient.isMockMode) {
    try {
      await apiClient.request(`/holds/${holdId}`, {
        method: 'DELETE',
      });
      return;
    } catch (err) {
      console.warn('Backend API unavailable for releasing hold:', err);
    }
  }
}
...
export const fetchHoldById = getHold;
