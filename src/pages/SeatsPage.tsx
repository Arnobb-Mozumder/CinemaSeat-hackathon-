...
import { createSeatHold, releaseHold, fetchHoldById } from '../api/holds';
...
export const SeatsPage: React.FC = () => {
    ...
    if (storedHoldStr) {
      try {
        ...
        if (expireTime > Date.now()) {
          // Verify with server if hold is still active
          fetchHoldById(storedHold.holdId)
            .then((serverHold) => {
              if (serverHold && serverHold.status === 'active' && new Date(serverHold.expiresAt).getTime() > Date.now()) {
                setHoldData(serverHold);
                setHeldSeatIds(serverHold.seatIds);
              } else {
            ...
            .catch(() => {
              // If hold fetch fails or hold is no longer active, clear storage
              sessionStorage.removeItem(storageKey);
              setHoldData(null);
              setHeldSeatIds([]);
            });
  ...
  const handleCancelHold = useCallback(async () => {
    ...
    if (holdData?.holdId) {
      try {
        await releaseHold(holdData.holdId);
      } catch (err) {
        console.warn('Failed to release hold on server:', err);
      }
    }
    ...
    await refetch();
  }, [showId, holdData, heldSeatIds, refetch]);
  ...
  const handleHoldExpired = useCallback(async () => {
    ...
    if (holdData?.holdId) {
      try {
        ...
        console.warn('Failed to release expired hold on server:', err);
      }
    }
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="glass-card rounded-2xl p-5 md:p-6 shadow-xl">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-16 rounded-lg bg-neutral-950/80 border border-emerald-900/50 overflow-hidden shrink-0">
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2 text-xs text-neutral-300 font-medium flex-wrap font-sans">
                <span className="font-bold text-white">{show.hallName}</span>
                <span className="text-emerald-900">•</span>
                <span className="text-emerald-400 font-bold font-mono">{show.startTime}</span>
                <span className="text-emerald-900">•</span>
                <span>{show.date}</span>
                <span className="text-emerald-900">•</span>
