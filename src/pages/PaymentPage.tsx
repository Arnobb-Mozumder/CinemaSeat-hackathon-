      ...
      if (targetBookingId) {
        ...
        try {
          ...
          if (isSuccessfulStatus(finalBooking.status)) {
            ...
            // Clear holds from sessionStorage on successful booking
            if (typeof window !== 'undefined') {
              try {
                Object.keys(window.sessionStorage).forEach((key) => {
                  if (key.startsWith('cinemaseat_hold_')) {
  ...
  return (
    <div className="space-y-6 pb-20 w-full mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-6">
            <Card className="glass-card shadow-xl">
              <CardHeader className="border-b border-emerald-950/60 pb-4">
                <div className="flex items-center justify-between gap-2 mb-2">
                  <Badge variant="primary" size="sm" className="bg-emerald-500 text-black font-extrabold">
                    {ctx.format || 'Standard'}
                  </Badge>
                  <span className="text-xs font-semibold text-emerald-400 font-mono">Locked Seats</span>
                </div>
            ...
            <Card className="glass-card shadow-2xl sticky top-20">
              <CardHeader className="border-b border-emerald-950/60 pb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <CardTitle className="text-lg font-bold text-white font-sans">Your Details</CardTitle>
                    <p className="text-[11px] text-neutral-400">We only need a few basics.</p>
                  </div>
                </div>
              </CardHeader>

              <CardContent className="py-6 space-y-5 font-sans">
                {/* Name */}
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full bg-neutral-950/80 border border-emerald-900/50 rounded-lg px-3 py-2.5 text-sm text-white placeholder-neutral-500 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/50"
                  />
                </div>
                ...
                {/* Total */}
                <div className="p-4 rounded-xl bg-emerald-950/40 border border-emerald-900/60 space-y-2">
                ...
                {/* Security */}
                <div className="p-3.5 rounded-xl bg-emerald-950/20 border border-emerald-900/40 text-xs text-neutral-400 space-y-1.5">
                ...
                <Button
                  variant="primary"
                  className="w-full py-3.5 text-sm font-extrabold bg-emerald-500 hover:bg-emerald-400 text-black shadow-xl shadow-emerald-950/60"
      ...
      {paymentStatus === 'pending' && (
        <div className="p-8 sm:p-12 rounded-2xl glass-card text-center space-y-6 shadow-2xl animate-fade-in">
          <div className="relative w-16 h-16 mx-auto flex items-center justify-center">
            <div className="absolute inset-0 rounded-full border-4 border-emerald-500/20 animate-ping" />
            <div className="w-16 h-16 rounded-full bg-emerald-950/80 border border-emerald-700/80 flex items-center justify-center text-emerald-400 shadow-xl shadow-emerald-950/50">
              <Loader2 className="w-8 h-8 animate-spin" />
            </div>
          </div>
          <h2 className="text-xl font-bold text-white tracking-tight font-sans">Payment processing...</h2>
